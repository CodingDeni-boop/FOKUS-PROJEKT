...
def annotate_video_from_csv(video_path, predictions_csv, output_path, label_column='prediction', frame_offset=15, true_labels_csv=None, true_label_column='label'):

    # Load predictions
    df = pd.read_csv(predictions_csv)

    predictions = df[label_column].values

    # Load true labels if provided
    true_labels = None
    if true_labels_csv:
        true_df = pd.read_csv(true_labels_csv)
        true_labels = true_df[true_label_column].values

    annotate_video_with_predictions(video_path, predictions, output_path, frame_offset=frame_offset, true_labels=true_labels)


annotate_video_from_csv(
    video_path="OriginalVideos/2_camera-1_0_synced.avi",
    predictions_csv="../model/rf_prediction.csv",
    output_path="annotated_video_2.mp4",
    label_column="label",
    frame_offset=15,
    true_labels_csv="../model/rf_test.csv",
